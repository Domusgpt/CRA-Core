...
class CRARuntime:
    ...
    def resolve(self, goal: str, target_platforms: Optional[List[str]] = None, risk_tier: str = "low",
                ...
                trace_ids: Optional[TraceIds] = None, token: Optional[str] = None,
                refresh_context: bool = False) -> Dict:
        ...
        try:
            resolution = self._build_resolution(risk_tier, trace_emitter=trace_emitter, refresh_context=refresh_context)
    ...
    def _build_resolution(
        ...
        for block in self.atlas_loader.context_blocks():
            managed_block, status = self.context_manager.enforce(block, force_refresh=refresh_context)
            context_blocks.append(managed_block)
            if not trace_emitter:
                continue
            payload = {
                "block_id": managed_block.block_id,
                "status": status,
                "expires_at": managed_block.expires_at,
                "ttl_seconds": managed_block.ttl_seconds,
            }
            if status == "expired":
                trace_emitter.emit(
                    "trace.artifact.redacted",
                    {**payload, "reason": "ttl_expired"},
                    severity="warn",
                )
            elif status == "created":
                trace_emitter.emit("trace.artifact.created", payload)
            elif status == "refreshed":
                trace_emitter.emit(
                    "trace.artifact.updated",
