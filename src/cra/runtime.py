...
class CRARuntime:
    ...
    def resolve(self, goal: str, target_platforms: Optional[List[str]] = None, risk_tier: str = "low",
        ...
        try:
            resolution = self._build_resolution(risk_tier, trace_emitter=trace_emitter)
    ...
    def _build_resolution(self, risk_tier: str, trace_emitter: TraceEmitter | None = None) -> Resolution:
        context_blocks: List[ContextBlock] = []
        for block in self.atlas_loader.context_blocks():
            managed_block, status = self.context_manager.enforce(block)
