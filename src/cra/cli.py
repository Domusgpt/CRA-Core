...
def conformance(
    fixture: Path = typer.Option(
        Path("traces/fixtures/adapter_scopes.json"), help="Adapter scope fixture JSON"
    ),
    trace_file: Optional[Path] = typer.Option(
        None, help="Optional TRACE JSONL to validate during certification"
    ),
    fail_fast: bool = typer.Option(False, help="Stop on first failure"),
) -> None:
    """Run lightweight conformance checks against fixtures and trace replays."""

    runner = ConformanceRunner()
    results = runner.run(fixture_path=fixture, trace_file=trace_file, fail_fast=fail_fast)
