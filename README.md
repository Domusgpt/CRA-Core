...
7. Run lightweight conformance checks against adapter scope fixtures (and optional TRACE replays)
   ```bash
   python -m cra.cli conformance --fixture traces/fixtures/adapter_scopes.json
   python -m cra.cli conformance --fixture traces/fixtures/adapter_scopes.json --trace-file traces/$(cat traces/latest).jsonl
   ```
