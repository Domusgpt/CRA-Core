...
- `cra conformance` runs adapter scope fixture checks and optional TRACE replay validation for certification harnesses

## Runtime Checks
- Enforce TTL on context blocks via the persisted context registry; expired blocks must be redacted in responses and emit `trace.artifact.redacted` events with reasons
- Emit `trace.artifact.created|updated` when context blocks are first issued or refreshed due to content drift
- Manual context refresh controls (`cra context-refresh` or `cra resolve --refresh-context`) must reset TTL state and emit creation/updated events on subsequent resolves
- Verify allowed_actions align with Atlas adapter schemas, rate limits, approval flags, and required scopes recorded in the Atlas manifest
- Verify action required scopes are enforced via identity tokens; missing scopes or invalid tokens emit `trace.action.policy.denied` and halt invocation
- Confirm denylist patterns are present for medium/high risk tiers
- Action invocation emits `trace.action.granted|invoked|completed|failed` plus governance events (`trace.action.policy.denied|pending_approval|rate_limited`) capturing license/approval/rate limit outcomes and rate windows
...
- `traces/fixtures/adapter_scopes.json` provides a reference fixture for adapter scope/risk/approval expectations in certification harnesses
